...
import { tracer, logger } from '@oneatlas/shared';
...
${repairInstructions}`;
    ...
    try {
      ...
      try {
        ...
        return { success: false, error: `Repaired content still failed validation: ${error.message}` };
      }
    } catch (error) {
      const routerError = error instanceof Error ? error : new Error(String(error));
      this.router.recordFailure?.(providerName, routerError);
