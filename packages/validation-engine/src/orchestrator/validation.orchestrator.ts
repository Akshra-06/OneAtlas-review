import { z } from 'zod';
import { ResponseRecovery } from '../recovery/response.recovery';
import { OutputFormatter } from '@oneatlas/shared';
import { calculateBackoff, delay, withTimeout, DEFAULT_RETRY_CONFIG } from '../recovery/fallback.strategies';
import type { RetryConfig } from '../recovery/fallback.strategies';
import type { AppUnderstanding, EntityNode } from '@oneatlas/shared';
import type { ModelRouter } from '@oneatlas/shared';
import { logger, tracer } from '@oneatlas/shared';

// Gateway types for provider interface compatibility
export interface AIRequest<T = any, E = any> {
  prompt: string;
  systemPrompt?: string;
  modelTier?: any; // ModelTier
  temperature?: number;
  maxTokens?: number;
...
export class ValidationOrchestrator {
  ...
  async executeWithValidation<T>(
    ...
    while (attempt <= retryConfig.maxAttempts) {
      ...
      const { provider, name: providerName } = this.router.getProviderForTask(this.taskType);
      ...
      try {
        ...
        const response: AIResponse = await withTimeout(
        ...
        try {
          ...
          const { GraphValidator, SemanticGraphError } = await import('./graph.validator');
          try {
          ...
          if (recoveryResult.success && recoveryResult.data) {
            this.router.recordSuccess?.(providerName);
            return { success: true, data: recoveryResult.data as T };
          } else {
        ...
        this.router.recordFailure?.(providerName, error as Error);
