...
import { MutationClassifier } from './mutation.classifier';
import type { AppUnderstanding } from '@oneatlas/shared';
import type { MutationPatch } from '@oneatlas/validation-engine';
import { logger } from '../../shared/utils/logger';
import { UnderstandingOrchestrator } from '../orchestrator/understanding.orchestrator';
...
export class MutationEngine {
  ...
  async mutate(currentSpec: AppUnderstanding, mutationPrompt: string, sessionId: string = 'default-session'): Promise<MutationResult> {
    ...
    try {
      ...
      const { GraphValidator } = await import('@oneatlas/validation-engine');
