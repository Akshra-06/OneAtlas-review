...
import { AppCategory } from '../detector/apptype.detector';
import { resolveCategory } from '../normalizer/category.normalizer';
import { z } from 'zod';
...
export class AppTypeExtractor {
  ...
  async extract(prompt: string): Promise<AppCategory> {
    ...
    const result = await orchestrator.executeWithValidation<{ category: string }>(
      {
        prompt,
