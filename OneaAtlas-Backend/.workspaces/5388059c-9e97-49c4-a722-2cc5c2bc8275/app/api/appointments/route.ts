...
export async function GET(request: NextRequest) {
  ...
  if (USE_MOCK_STORAGE) {
    ...
    const result = await mockStorage.findMany('appointment', where, {
      skip: (page - 1) * limit,
      take: limit,
    });
...
export async function POST(request: NextRequest) {
  try {
    const validated = AppointmentCreateSchema.parse(await request.json());
    const tenantId = await getTenantId(request);

    if (USE_MOCK_STORAGE) {
      const data = await mockStorage.create('appointment', {
        ...validated,
        tenantId,
      });
